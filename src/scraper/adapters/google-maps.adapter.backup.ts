    ...
    async scrape(params: ScrapeParams): Promise<RawData[]> {
        ...
        try {
            ...
            if (this.config.geolocation) {
                const { latitude, longitude, zoom = 15 } = this.config.geolocation;
                // 使用地理位置参数构建 URL
                // 格式: https://www.google.com/maps/search/{query}/@{lat},{lng},{zoom}z
                searchUrl = `https://www.google.com/maps/search/${encodeURIComponent(params.query)}/@${latitude},${longitude},${zoom}z`;
                logger.info(`[GoogleMaps] 使用地理位置搜索: lat=${latitude}, lng=${longitude}, zoom=${zoom}`);
            } else {
                // 使用默认关键词搜索
                searchUrl = `https://www.google.com/maps/search/${encodeURIComponent(params.query)}`; logger.info(`[GoogleMaps] 使用关键词搜索`);
            }
